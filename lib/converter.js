  ...
  static convert(mod, yargs) {
    ...
    for (var p = 0; p < mod.sequence.length; p = nextP) {
      ...
      for (var r = startRow; r < 64; r++) {
        ...
        startRow = 0; // pick up speed first
          ...
          switch (row.command) {
            ...
            case 0xE:
              switch ((row.parameter & 0xF0) >>> 4) {
                case 0x6:
                  var count = row.parameter & 0x0F; // set loop
        ...
        for (var t = 0; t < 4; t++) {
          ...
          if (row.note != 0) {
            ...
            if (row.command == 0x3 || row.command == 0x5) {
              targetPeriod[t] = _classStaticPrivateFieldSpecGet(this, Converter, _periodTable)[finetune][_classStaticPrivateFieldSpecGet(this, Converter, _periodTable)[0].indexOf(row.note)];
            } else {
              trackPeriod[t] = _classStaticPrivateFieldSpecGet(this, Converter, _periodTable)[finetune][_classStaticPrivateFieldSpecGet(this, Converter, _periodTable)[0].indexOf(row.note)];
            }
          }
          ...
          switch (row.command) {
            case 0x0:
              if (row.parameter != 0x00) {
                var arpOffsets = [0, (row.parameter & 0xF0) >>> 4, row.parameter & 0x0F];
                ...
                for (var step = 0; step < vBlankSpeed; step++) {
                  ...
                  switch (arpStep) {
                    ...
                    default:
                      var finetune = mod.instruments[trackInstrumentNumber[t] - 1].finetune;
                      periodData[t].push(_classStaticPrivateFieldSpecGet(this, Converter, _periodTable)[finetune][_classStaticPrivateFieldSpecGet(this, Converter, _periodTable)[finetune].indexOf(_classStaticPrivateFieldSpecGet(this, Converter, _periodTable)[finetune].find(function (period) {
                        return period <= trackPeriod[t];
                      })) + arpOffsets[arpStep]]);
                  }
                }
              } else {
            ...
            case 0x1:
              ...
              for (var step = 0; step < vBlankSpeed - 1; step++) {
                ...
                if (trackPeriod[t] < _classStaticPrivateFieldSpecGet(this, Converter, _MIN_PERIOD)) {
                  trackPeriod[t] = _classStaticPrivateFieldSpecGet(this, Converter, _MIN_PERIOD);
                }
            ...
            case 0x2:
              ...
              for (var step = 0; step < vBlankSpeed - 1; step++) {
                ...
                if (trackPeriod[t] > _classStaticPrivateFieldSpecGet(this, Converter, _MAX_PERIOD)) {
                  trackPeriod[t] = _classStaticPrivateFieldSpecGet(this, Converter, _MAX_PERIOD);
                }
            ...
            case 0x6:
              ...
              for (var step = 0; step < vBlankSpeed - 1; step++) {
                periodData[t].push(trackPeriod[t] + Math.floor(_classStaticPrivateFieldSpecGet(this, Converter, _vibratoTable)[trackVibrato[t].position] * trackVibrato[t].depth / 128));
    ...
    console.log("\nRestart from tick:", restartTick); // zip the note trigger, instrument, volume and period data for each channel

    var trackData = [[], [], [], []];
    var initDma = 0X0000; // nove the note trigger data one forward, sp we can set dma stop flag on the previous tick
      ...
      for (var t = 0; t < 4; t++) {
        ...
        if (instrumentData[t][tick] != 0 || periodChange < -128 || periodChange > 127) {
          ...
          if (periodChange < -128 || periodChange > 127) {
            console.error("Invalid period change!"); //process.exit(1);	
          }

          word |= periodChange & 0x00FF; // no new note
        }
    ...
    if (yargs.compress) {
      ...
      for (var t = 0; t < 4; t++) {
        ...
        console.log("\nFinal LZ pass\n------");
        console.log("Optimal sliding window size is:", upperBound); // compress again with the optimal window size because it might not be the last pass
    ...
    for (var t = 0; t < 4; t++) {
      ...
      outData.push(0xFF);
      outData.push(0xFF);
      console.log("Track", t, "has", wordSet.size, "different values."); // restart pointer
    ...
    outData.push(0xFF);
    outData.push(0xFF); // Initial DMACON value
    ...
    outData.push(0xFF);
    outData.push(0xFF); // add the decompression buffer sizes
...
var _vibratoTable = {
  writable: true,
  value: [0, 24, 49, 74, 97, 120, 141, 161, 180, 197, 212, 224, 235, 244, 250, 253, 255, 253, 250, 244, 235, 224, 212, 197, 180, 161, 141, 120, 97, 74, 49, 24, 0, -24, -49, -74, -97, -120, -141, -161, -180, -197, -212, -224, -235, -244, -250, -253, -255, -253, -250, -244, -235, -224, -212, -197, -180, -161, -141, -120, -97, -74, -49, -24]
};
var _periodTable = {
  writable: true,
  value: [[// ; Tuning 0, Normal
  856, 808, 762, 720, 678, 640, 604, 570, 538, 508, 480, 453, 428, 404, 381, 360, 339, 320, 302, 285, 269, 254, 240, 226, 214, 202, 190, 180, 170, 160, 151, 143, 135, 127, 120, 113], [// ; Tuning 1
  850, 802, 757, 715, 674, 637, 601, 567, 535, 505, 477, 450, 425, 401, 379, 357, 337, 318, 300, 284, 268, 253, 239, 225, 213, 201, 189, 179, 169, 159, 150, 142, 134, 126, 119, 113], [// ; Tuning 2
  844, 796, 752, 709, 670, 632, 597, 563, 532, 502, 474, 447, 422, 398, 376, 355, 335, 316, 298, 282, 266, 251, 237, 224, 211, 199, 188, 177, 167, 158, 149, 141, 133, 125, 118, 112], [// ; Tuning 3
  838, 791, 746, 704, 665, 628, 592, 559, 528, 498, 470, 444, 419, 395, 373, 352, 332, 314, 296, 280, 264, 249, 235, 222, 209, 198, 187, 176, 166, 157, 148, 140, 132, 125, 118, 111], [// ; Tuning 4
  832, 785, 741, 699, 660, 623, 588, 555, 524, 495, 467, 441, 416, 392, 370, 350, 330, 312, 294, 278, 262, 247, 233, 220, 208, 196, 185, 175, 165, 156, 147, 139, 131, 124, 117, 110], [// ; Tuning 5
  826, 779, 736, 694, 655, 619, 584, 551, 520, 491, 463, 437, 413, 390, 368, 347, 328, 309, 292, 276, 260, 245, 232, 219, 206, 195, 184, 174, 164, 155, 146, 138, 130, 123, 116, 109], [// Tuning 6
  820, 774, 730, 689, 651, 614, 580, 547, 516, 487, 460, 434, 410, 387, 365, 345, 325, 307, 290, 274, 258, 244, 230, 217, 205, 193, 183, 172, 163, 154, 145, 137, 129, 122, 115, 109], [// Tuning 7
  814, 768, 725, 684, 646, 610, 575, 543, 513, 484, 457, 431, 407, 384, 363, 342, 323, 305, 288, 272, 256, 242, 228, 216, 204, 192, 181, 171, 161, 152, 144, 136, 128, 121, 114, 108], [// Tuning -8
  907, 856, 808, 762, 720, 678, 640, 604, 570, 538, 508, 480, 453, 428, 404, 381, 360, 339, 320, 302, 285, 269, 254, 240, 226, 214, 202, 190, 180, 170, 160, 151, 143, 135, 127, 120], [// Tuning -7
  900, 850, 802, 757, 715, 675, 636, 601, 567, 535, 505, 477, 450, 425, 401, 379, 357, 337, 318, 300, 284, 268, 253, 238, 225, 212, 200, 189, 179, 169, 159, 150, 142, 134, 126, 119], [// Tuning -6
  894, 844, 796, 752, 709, 670, 632, 597, 563, 532, 502, 474, 447, 422, 398, 376, 355, 335, 316, 298, 282, 266, 251, 237, 223, 211, 199, 188, 177, 167, 158, 149, 141, 133, 125, 118], [// Tuning -5
  887, 838, 791, 746, 704, 665, 628, 592, 559, 528, 498, 470, 444, 419, 395, 373, 352, 332, 314, 296, 280, 264, 249, 235, 222, 209, 198, 187, 176, 166, 157, 148, 140, 132, 125, 118], [// Tuning -4
  881, 832, 785, 741, 699, 660, 623, 588, 555, 524, 494, 467, 441, 416, 392, 370, 350, 330, 312, 294, 278, 262, 247, 233, 220, 208, 196, 185, 175, 165, 156, 147, 139, 131, 123, 117], [// Tuning -3
  875, 826, 779, 736, 694, 655, 619, 584, 551, 520, 491, 463, 437, 413, 390, 368, 347, 328, 309, 292, 276, 260, 245, 232, 219, 206, 195, 184, 174, 164, 155, 146, 138, 130, 123, 116], [// Tuning -2
  868, 820, 774, 730, 689, 651, 614, 580, 547, 516, 487, 460, 434, 410, 387, 365, 345, 325, 307, 290, 274, 258, 244, 230, 217, 205, 193, 183, 172, 163, 154, 145, 137, 129, 122, 115], [// Tuning -1
