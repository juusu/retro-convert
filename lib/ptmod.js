...
class Module {
  constructor(data) {
    ...
    for (var instrumentNumber = 1; instrumentNumber < 32; instrumentNumber++) {
      var instrument = new Instrument(data.slice((instrumentNumber - 1) * INSTRUMENT_RECORD_SIZE + 20, instrumentNumber * INSTRUMENT_RECORD_SIZE + 20), instrumentNumber);
    ...
    for (var pattern = 0; pattern < songLength; pattern++) {
      var patternNumber = data.readUInt8(952 + pattern);
      tempSequence.push(patternNumber);
      maxPattern = Math.max(patternNumber, maxPattern);
    }

    if (!this.hundredPatterns && maxPattern > 63) {
      console.error("Too many patterns found for M.K. (64 pattern) module:", maxPatterns);
    }

    this.patterns = [];

    for (var patternNumber = 0; patternNumber <= maxPattern; patternNumber++) {
      var pattern = new Pattern(data.slice(1084 + patternNumber * 1024, 1084 + (patternNumber + 1) * 1024));
